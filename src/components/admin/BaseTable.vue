  ...
  <div class="text-base">
    ...
    <div v-if="loading" class="animate-pulse space-y-3">
      <div v-for="i in 5" :key="i" class="flex gap-4">
        <div
          v-for="j in columnKeys.length + ($slots.actions ? 1 : 0)"
          :key="j"
          class="h-10 flex-1 rounded bg-gray-200"
        />
      </div>
    </div>

    <!-- 错误状态 -->
    <div v-else-if="error" class="rounded-lg bg-red-50 p-8 text-center text-red-500">
      <p class="mb-1 text-lg font-medium">加载失败</p>
      <p class="text-sm">{{ error }}</p>
    </div>

    <!-- 表格（仅表格区域可横向滚动） -->
    <div v-else class="overflow-x-auto">
      <table class="w-full border-collapse" :class="{ 'border border-gray-200': tableProps.bordered }">
        <caption v-if="tableProps.caption" class="pb-2.5 text-left text-2xl font-bold">
          {{
            tableProps.caption
          }}
        </caption>
        <thead>
          <tr class="bg-gray-100 text-left">
            ...
            <th
              v-if="$slots.actions"
              class="sticky right-0 z-10 bg-gray-100 p-3 font-medium whitespace-nowrap"
              :class="{
                'border border-gray-200': tableProps.bordered,
                'border-b-2 border-gray-200': !tableProps.bordered,
              }"
        ...
        <tbody>
          ...
          >
            ...
            >
              ...
              <slot v-if="$slots[key]" :name="key" :row="item" :value="item[key]" />
              <template v-else>
          ...
          <tr v-if="pagedList.length === 0 && !loading">
            <td :colspan="columnKeys.length + ($slots.actions ? 1 : 0)" class="p-8 text-center text-gray-400">
    ...
    <!-- 分页（固定不随表格滚动） -->
    <div v-if="hasPagination && !loading && !error" class="mt-5 flex flex-wrap items-center justify-between gap-4">
      <!-- 每页条数 -->
      <div class="flex items-center gap-2 text-sm text-gray-600">
        <span>每页</span>
        <select
          :value="pagedData.size"
          class="rounded border border-gray-300 bg-white px-2 py-1 outline-none"
      ...
      <div class="flex items-center gap-2">
        <button
          class="rounded border border-gray-300 px-3 py-1.5 text-sm transition-colors hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-40"
        ...
        <template v-for="p in pageTotalPages" :key="p">
          <button
            ...
            class="h-9 w-9 rounded text-sm transition-colors"
            :class="p === pagedData.page ? 'bg-[#5268bc] text-white' : 'border border-gray-300 hover:bg-gray-100'"
