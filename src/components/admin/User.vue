...
<template>
  ...
  <Teleport to="body">
    <Transition name="modal-fade">
      ...
      >
        <div class="max-h-[90vh] w-[420px] max-w-[90vw] overflow-y-auto rounded-xl bg-white p-6 shadow-2xl">
          <h2 class="mb-4 text-xl font-bold">修改用户信息</h2>
          <form @submit.prevent="saveUser" class="flex flex-col gap-4">
            <div>
              ...
              <input
                ...
                type="text"
                disabled
                class="w-full rounded border border-gray-300 bg-gray-100 px-3 py-2 text-gray-400 outline-none"
              />
            </div>
            <div>
            ...
            <div>
              <label class="mb-1 block text-sm font-medium">用户 QQ</label>
              <input
                v-model="selectedUser.userQQ"
                type="text"
                required
                placeholder="修改 QQ"
                class="w-full rounded border border-gray-300 px-3 py-2 outline-none focus:border-[#5268bc]"
            ...
            <div>
              <label class="mb-1 block text-sm font-medium">注册时间</label>
              <input
                v-model="selectedUser.addtime"
                type="datetime-local"
                class="w-full rounded border border-gray-300 px-3 py-2 outline-none focus:border-[#5268bc]"
              />
            </div>
            <div>
              <label class="mb-1 block text-sm font-medium">用户角色</label>
              <select
            ...
            <div>
              <label class="mb-1 block text-sm font-medium">账号状态</label>
              <select
