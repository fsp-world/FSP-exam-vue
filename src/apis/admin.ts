import type { ConfigItem, FetchResponse, IPagination, User, UserUpdate } from '@/types';
import type {
  SurveySlot,
  SurveyInfoItem,
  NewSurvey,
...
export const addUser = (data: User) => request.post('/admin/user', JSON.stringify(data));
export const updateUser = (data: UserUpdate) => request.put('/admin/user', JSON.stringify(data));
export const deleteUser = (id: number) => request.delete('/admin/user', { data: JSON.stringify({ id }) });

// whitelist
export const getWhitelist = (data: IPagination) => request.get('/admin/whitelist', { params: data });

// survey
export const getSurveys = (): Promise<FetchResponse<SurveyInfoItem[]>> => request.get('/admin/surveys');
export const getSurvey = async (id: number): Promise<FetchResponse<AdminViewSurvey>> => {
  try {
...
export const modSurveyMetaData = (data: NewSurvey): Promise<FetchResponse<any>> =>
  request.post('/admin/survey/update', JSON.stringify(data));

// question
export const addQuestionAPI = (data: UploadAddQuestion) => request.post('/admin/question/add', JSON.stringify(data));
export const editQuestionAPI = (data: UploadEditQuestion) => request.post('/admin/question/edit', JSON.stringify(data));
export const delQuestionAPI = (id: number) => request.post('/admin/question/delete', JSON.stringify({ id }));
...
export const responseDetail = async (id: number): Promise<FetchResponse<AdminReviewSurvey>> => {
  try {
    const response: FetchResponse<AdminReviewSurvey> = await request.get('/admin/detail/' + id);
    response.data.data.questions = sortQuestion(response.data.data.questions);
    return response;
  } catch (error) {
    console.error('Error fetching survey:', error);
...
export const detailScore = (data: {
  ...
  questionId: number;
  responseId: number;
}): Promise<FetchResponse<any>> => request.post('/admin/detail_score', JSON.stringify(data));

// slot
export const addSlotAPI = (data: SurveySlot) => request.post('/admin/slot/add', JSON.stringify(data));
export const setSlotAPI = (data: SurveySlot) => request.post('/admin/slot/set', JSON.stringify(data));
export const delSlotAPI = (data: SurveySlot) => request.post('/admin/slot/delete', JSON.stringify(data));

// guarantee
export const getGuaranteeAPI = (data: IPagination) => request.get('/admin/guarantee/get', { params: data });

// config
export const getConfig = (data: string = '') => request.get('/admin/config/get', { params: { key: data } });
export const getConfigs = (data: IPagination) => request.get('/admin/config/get', { params: data });
